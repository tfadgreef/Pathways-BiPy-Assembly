...
% UV and FL-traces at different concentrations. 
...
N=7;
Filename=cell(N,1);
Filename{1,1}='FitResults_n8_Ct1e-06.txt';
Filename{2,1}='FitResults_n8_Ct2.5e-06.txt';
Filename{3,1}='FitResults_n8_Ct5e-06.txt';
Filename{4,1}='FitResults_n8_Ct1e-05.txt';
Filename{5,1}='FitResults_n8_Ct2.5e-05.txt';
Filename{6,1}='FitResults_n8_Ct5e-05.txt';
Filename{7,1}='FitResults_n8_Ct0.0001.txt';
...
%     
    ...
    CDArray_Tempor=flipud(DataArray_Temp.data(:,2));
    ...
    UVArray_Tempor=flipud(DataArray_Temp.data(:,3));
    ...
    if f <= 5
        FLArray_Tempor=flipud(DataArray_Temp.data(:,4));
...
ylabel('Molar Circular Dichroism (1/(M*cm))')
...
xlabel('Temperature (K)')
ylabel('Molar Fluorescence Intensity (a.u.)')
...
h_colbar=colorbar('FontSize',12,'YTick',log10(logspace(-6,-4,3)),'YTickLabel',logspace(-6,-4,3));
...
print(h_fig,'-depsc','-r300','-tiff','Figure 2b')
% cd(CurrDom)